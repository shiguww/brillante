...
class KDM {
  ...
  public get(): IKDM {
    return IKDM.parse({
      tables: this.tables.map(([name, table]) => [name, table.get()])
    });
  }
  ...
  private registerStringIfNotExists(_string: string | KDMStringPointer): void {
  ...
  private prebuild(): void {
    ...
    this.tables.forEach(([name, table]) => {
      ...
      this.registerStringIfNotExists(name);
    });

    this.parameters.forEach((p) => this.registerStringIfNotExists(p.name));

    // Assigning IDs
    this.types.forEach((t) => t.OID = this.id++);
    ...
    this.parameters.forEach((p) => p.heading.uid.set(this.id++));
